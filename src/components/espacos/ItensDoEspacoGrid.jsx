...
export default function ItensDoEspacoGrid({
    ...
    const itensFiltrados = useMemo(() => {
        ...
        return itens.filter(item => {
            const correspondeUnidade = !tipoUnidadeSelecionada || Number(item.tipoUnidadeMedida) === Number(tipoUnidadeSelecionada);
    ...
    return (
        <div className={`space-items-manager ${menuAbertoId ? 'space-items-manager-open' : ''}`}>
            <div className="space-items-heading">
                <h2 className="space-items-title">{espaco?.nome}</h2>
